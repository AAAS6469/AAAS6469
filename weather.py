...
def write_to_screen():
    ...
    logging.info("epd7in3f WeatDemo")
    epd = epd7in3f.EPD()
    ...
    # === 気温 ===
    if float(temp)>temp_HOT:
        temp_color = red
    elif float(temp)<temp_COLD:
        temp_color = blue
    else :
        temp_color=black
        
    draw.text((275, 8), str(temp) + '°C', font=font160, fill=temp_color)  # 気温
    ...
    if len(str(wather))>10:
        draw.text((20, 265), str(wather)[10:], font=font22, fill=0)
    draw.text((30, 240), str(wather)[0:10], font=font22, fill=0)
    # === アイコン + 日付 ===

    # === 降水確率 ===
    draw.text((275, 220), '降水確率', font=font22, fill=blue)
    draw.rectangle((375, 280, 372, 195), fill=0)
    draw.text((390, 200), '00-06時', font=font22, fill=0)
    draw.text((400, 240), str(tsukumijima['forecasts'][0]['chanceOfRain']['T00_06']), font=font24, fill=blue)
    draw.text((490, 200), '06-12時', font=font22, fill=0)
    draw.text((500, 240), str(tsukumijima['forecasts'][0]['chanceOfRain']['T06_12']), font=font24, fill=blue)
    draw.text((590, 200), '12-18時', font=font22, fill=0)
    draw.text((600, 240), str(tsukumijima['forecasts'][0]['chanceOfRain']['T12_18']), font=font24, fill=blue)
    draw.text((690, 200), '18-24時', font=font22, fill=0)
    draw.text((700, 240), str(tsukumijima['forecasts'][0]['chanceOfRain']['T18_24']), font=font24, fill=blue)
    # === 降水確率 ===
